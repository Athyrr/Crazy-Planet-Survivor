    ...
    where TItem : UIViewItemBase
{
    ...
    private Material _actionOutlineMat;
    ...
    /// plus a colored outline driven on the button image's UI outline shader (_OutlineColor =
    ...
    protected void SetPurchaseFocused(bool focused)
    {
        ...
        // Outline feedback via the button image's UI outline shader, reusing the committed list
        // item's outline token so the button and its item read as one selected unit. The material is
        // cloned on first use so we don't repaint other UI graphics sharing the same outline asset
        // (e.g. the Back button, list items).
        if (ActionButton.image != null && ActionButton.image.material != null)
        {
            if (_actionOutlineMat == null)
            {
                _actionOutlineMat = new Material(ActionButton.image.material);
                ActionButton.image.material = _actionOutlineMat;
            }

            Color outlineColor = CpUISettings.ItemOutlineSelected;
            if (!canShow)
                outlineColor.a = 0f;
            _actionOutlineMat.SetColor(OutlineColorId, outlineColor);
        }

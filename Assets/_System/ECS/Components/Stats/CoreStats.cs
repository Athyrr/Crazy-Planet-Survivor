...
public struct CoreStats : IComponentData
{
    ...
    [Tooltip(
        "Damage reduction in % (0 = takes 100% damage, 0.2 = reduces 20% of entering damages)."
    )]
    [UIStat("Damage Reduc.", "{0:+0%;-0%;0%}")]
    ...
    [Tooltip("Global cooldown multiplier (0 = 100% du cd, 0.1 = -10% on spell cd).")] //todo
    [UIStat("Cooldown Reduc.", "{0:+0%;-0%;0%}")]
    public float GlobalCooldownReductionMultiplier;

    [Tooltip("Spell Size multiplier (Projectiles).")]
    ...
    [Header("Critical")]
    [Tooltip("Critical chance probability (0.0 to 1.0).")] //todo
    [UIStat("Crit Chance", "{0:0%}")]
    public float CritChance;

    [Tooltip("Critical damage multiplier (1.5 = 150%).")] //todo
